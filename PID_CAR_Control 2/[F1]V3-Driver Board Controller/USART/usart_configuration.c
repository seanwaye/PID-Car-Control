...
void Usart_UWB_init(void)
{
	...
	/* Enable USART3 clock for UWB */
	RCC_APB1PeriphClockCmd(USART_UWB_CLOCK, ENABLE);
	
	//Enable and set USART3 Interrupt the the second lowest priority
	NVIC_InitStructure.NVIC_IRQChannel = UWB_IRQn;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 2;
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
	
	/* Set GPIOs for USART3 - UWB */
	GPIO_InitStructure.GPIO_Pin = STM_UWB_TX;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(STM_UWB_PORT, &GPIO_InitStructure);
  
	GPIO_InitStructure.GPIO_Pin = STM_UWB_RX;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
  GPIO_Init(STM_UWB_PORT, &GPIO_InitStructure);  
	
	/* Usart3 Config */
	USART_InitStructure.USART_BaudRate = BAUDRATE_UWB;
	USART_InitStructure.USART_WordLength = USART_WordLength_8b;
	USART_InitStructure.USART_StopBits = USART_StopBits_1;
	USART_InitStructure.USART_Parity = USART_Parity_No;
	USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
	...
	USART_Init(USART_UWB_CHANNEL, &USART_InitStructure);   

	/* Set NVIC for UWB - USART3 */
	// Interrupt while receiving data
	USART_ITConfig(USART_UWB_CHANNEL, USART_IT_RXNE, ENABLE); //接收到信息中断
	USART_ITConfig(USART_UWB_CHANNEL, USART_IT_TXE, DISABLE);   //SEND到信息中断

	/* Enable USART3 */
	USART_Cmd(USART_UWB_CHANNEL, ENABLE);
}
